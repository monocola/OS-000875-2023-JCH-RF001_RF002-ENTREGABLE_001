...
export class ProfileComponent implements OnInit {
  ...
  loadCombox() {
    ...
    const getDatosPersonales = this.organigramaService.getDatosPersonales(
      this.profile.entidadId,
      this.profile.personaId
    );
  ...
  initialFrom() {
    ...
    if (this.datosPersonales.fechaNacimiento) {
      ...
      //this.fechaNac = Utils.formatFechaDate(this.datosPersonales.fechaNacimiento, 'DD/MM/YYYY');
    ...
    this.frm.patchValue({
      ...
      sindicato: this.datosPersonales.sindicato,
    });
  }
    ...
    if (this.frm.valid) {
      this.organigramaService.updateDatosPersonales(body).subscribe((res) => {
        if (res.status.success) {
          this.loadCombox();
          this.toast.showToast('Se actualizó con Éxito', 'success', 'Atención');
        } else {
  ...
  validaCampos() {
    ...
    console.log("FOTO", this.imgData64)
    return {
      entidadId: entidadId,
      personaId: personaId,
      telefono: telefono,

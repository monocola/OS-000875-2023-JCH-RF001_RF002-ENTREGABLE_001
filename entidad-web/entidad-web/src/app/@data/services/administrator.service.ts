...
import { RegistroPostulante } from '../model/registro.postulante';
...
export class AdministratorService implements AdministratorRepository {
  ...
  updateSolicitudPerson(
    ...
    solicitud.estadoSolicitud = Const.ENT_ESTADO_PENDIENTE;
    ...
    const tramaEnvio = {
      ...
      payload: {
        solicitud: {
          ...
          estadoSolicitud: Const.ENT_ESTADO_PENDIENTE,
        },
        ...
        listaSolicitudPersona: [
          // ---------------------------------- Persona Juridica ---------------------------------- //
          {
            ...
            rolEntidadId: 0,
            tipoPersona: Const.PER_TIPO_PERSONA_JUR,
            tipoDocumento: 6,
          ...
          // ---------------------------------- Persona Natural ---------------------------------- //
          {
            ...
            rolEntidadId: Const.R_ADMIN_ENTIDAD,
  ...
  verifyEmail(email: string) {
    const url = `${Const.API_ENTIDAD}v1/solicitudentidad/validarcorreo/${email}`;
    return this.http.get(url).pipe(
      map((response: any) => {
        if (response.status.success) {
          return response.payload.codigoConfirmacion;
        } else {
          throw new Error(response.status.error.messages[0]).message;
        }
      })
  ...
  validDuplicateRequest (ruc: number, doc: number) {
    const url = `${Const.API_ENTIDAD}v1/solicitudentidad/validarsolicitud/${ruc}/${doc}`;
    return this.http.get(url).pipe(
      map((response: any) => {
        if (response.status.success) {
          return response.payload.solicitudExistente;
        } else {
          throw new Error(response.status.error.messages[0]).message;
        }
      })
  ...
  insConfigMaestra(entidadId: number): Observable<any> {
    const url = `${Const.API_CONVOCATORIA}v1/maestraCabecera/activa/${entidadId}`;
    return this.http.get(url).pipe(
      map((response: any) => {
        if (response.status.success) {
          return response;
  ...
  registerPostulante(data: RegistroPostulante): Observable<any> {
    const url = `${Const.API_POSTULANTE}v1/postulante/interno`;

...
export class OrganoService implements OrganoRepository {
  ...
  searchOrganos(query: string): Observable<Organo[]> {
    const entidadId = this.authService.getCurrentUserValue.entidadId;
    const url = `${Const.API_ENTIDAD}v1/organigrama/filter?texto=${query}&entidadId=${entidadId}&tipo=${Const.TIPO_ORGANO}`;
  ...
  createOrUpdateOrgano(
    ...
    const tramaEnvio = {
      ...
      payload: {
        ...
        orden: 0,
        nivel: body.nivel,
        tipoOrganoUoId: Const.TIPO_ORGANO, // fijo

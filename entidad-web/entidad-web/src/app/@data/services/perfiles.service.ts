...
export class PerfilesService extends PerfilesRepository {
  ...
  getPerfiles(body?: any): Observable<any[]> {
    ...
    const params = requestFilter({
      ...
      estadoRevision: body?.estadoRevision,
      origenPerfil: body?.origenPerfil
    });
    ...
    const tramaEnvio = {
      ...
      payload: {
        perfil: {
          ...
          estadoRevision: body.estadoRevision || null
        },
      },
    };
  ...
  saveOrUpdateFormacion(body, regimenId, perfilId): Observable<any> {
    ...
    const tramaEnvio = {
      ...
      payload: {
        ...
        lstConocimientos: this.parseConocimientos(body, 'conocimientosTecnicos')
          .concat(this.parseConocimientos(body, 'cursosRequeridos'))
          .concat(this.parseConocimientos(body, 'programasRequeridos'))
          .concat(this.parseConocimientos(body, 'ofimatica'))
          .concat(this.parseConocimientos(body, 'idiomas'))
          .concat(this.parseConocimientos(body, 'conocimientosBasicos'))
      }
    };
    
    return this.http.post(url, tramaEnvio).pipe(
      map((response: ResponseRequest) => {
        if (response.payload.perfilId) {
          return response.payload;
        } else {
          throw new Error(response.status.error.messages[0]).message;
        }
  ...
  parseConocimientos(body, tipo: string): any[] {
    let result = [];
    if (body[tipo] != null && body[tipo].length !== 0) {
      result.push(...body[tipo].map(item => {
        return {
          perfilId: body.perfil,
          regimenLaboralId: body.regimen,
          perfilConocimientoId: item.perfilConocimientoId || null,
          tipoConocimientoId: item.tipoConocimientoId,
          maeConocimientoId: item.maeConocimientoId,
          descripcionConocimiento: item.descripcion,
          horas: item.horas,
          nivelDominioId: item.nivelDominioId,
  ...
  revisarPerfil (perfilIdentificacionId: number, estadoRevision: number) {
    const url = `${Const.API_CONVOCATORIA}v1/perfil/estadorevision?perfilId=${perfilIdentificacionId}&estadoRevision=${estadoRevision}`;
    return this.http.put(url, {}).pipe(
      map((response: ResponseRequest) => {
        if (response.status.success) {
          return response.payload;
  ...
  getPlantillaRegimen (regimen: string) {
    const entidadId = this.authenticationRepository.getCurrentUserValue.entidadId;
    const url = `${Const.API_CONVOCATORIA}v1/perfil/regimen/${regimen}/${entidadId}/download`;
    return this.http.get(url).pipe(
      map((response: ResponseRequest) => {
        if (response.status.success) {
          return response.payload;
  ...
  uploadPlantilla (request: any) {
    const url = `${Const.API_CONVOCATORIA}v1/cargamasiva/perfil/upload`;

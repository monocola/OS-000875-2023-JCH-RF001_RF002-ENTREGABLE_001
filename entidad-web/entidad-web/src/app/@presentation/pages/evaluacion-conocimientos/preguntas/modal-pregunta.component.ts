...
@Component({
  selector: 'serv-talento-preguntas',
  templateUrl: './modal-pregunta.component.html',
  styleUrls: ['./modal-pregunta.component.scss'],
})
export class ModalCrearPreguntaComponent implements OnInit, OnDestroy {
  const = Const;
  ...
  async ngOnInit() {
    this.dialogRef.updateSize('50%', '99%');
    this.fileName = null;
          ...
          img.onload = () => {
            ...
            if (!(height <= 512 && width <= 512)) {
              ...
              if (!pattern.test(reqFileName)) {
                ...
                const extension = file.name.split('.')[file.name.split('.').length - 1];
                let today = new Date();
                let nomArchivo = reqFileName.toLowerCase() + today.getTime().toString().slice(8);
                let archivoFile = new File([file], nomArchivo + '.' + extension);
                this.fileName = archivoFile.name.toLowerCase();

                getBase64(archivoFile).then((data: string) => {

...
export class MaterialTableCheckboxComponent implements OnInit, AfterViewInit {
  ...
  @Output() inputItemAction: EventEmitter<any> = new EventEmitter<any>();
  ...
  emitPuntaje(row: any, input: any) {
    row.puntajePregunta = input.target.value;
    this.inputItemAction.emit(row);
  }
}

...
export class Utils {
  ...
  static formatDateTimeFechaString(fecha, formato) {
    let dateMomentObject = moment(fecha, 'DD/MM/YYYY HH:mm:ss');
    return dateMomentObject.format(formato);
  }
  ...
  static rountTo(num: number, places: number) {
...
export function armarPayload<T>(payload: T): any {
  return {
    trace: {
      traceId: 'string',
    },

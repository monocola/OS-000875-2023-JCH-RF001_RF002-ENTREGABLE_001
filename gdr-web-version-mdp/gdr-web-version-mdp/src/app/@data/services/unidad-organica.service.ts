...
export class UnidadOrganicaService
  ...
  getUnidadOrganicaSup(tipoOrganoId?: number): Observable<any> {
    ...
    const url = `${Const.API_ENTIDAD}v1/organigrama/unidadorganica/superior?entidadId=${entidadId}` + (tipoOrganoId ? `&tipoOrganoId=${tipoOrganoId}` : '');
    return this.http.get(url).pipe(
      map((response: ResponseRequest) => {
        if (!response.status.success) {
          throw new Error(response.status.error.messages[0]).message;
  ...
  getUnidadOrganicaCbo(tipoOrganoId?: number, unidadOrganicaSuperiorId?: number): Observable<any> {
    let entidadId = this.authService.getCurrentUserValue.entidadId;
    let monitoreo = Utils.obtenerSiEsPantallMonitoreo();
    if(monitoreo) {
      entidadId = JSON.parse(sessionStorage.getItem('entidad')).entidadId;
    ...
    const url = `${Const.API_ENTIDAD}v1/organigrama/unidadorganica/combo?entidadId=${entidadId}`
  ...
  getPuestos(descripcion?: string): Observable<any> {
    let entidadId = this.authService.getCurrentUserValue.entidadId;
    let monitoreo = Utils.obtenerSiEsPantallMonitoreo();
    if(monitoreo) {
      entidadId = JSON.parse(sessionStorage.getItem('entidad')).entidadId;
    }
    const url = `${Const.API_ENTIDAD}v1/puesto/filtrar?entidadId=${entidadId}`

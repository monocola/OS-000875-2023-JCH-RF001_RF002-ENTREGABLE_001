...
@Component({
  selector: 'serv-talento-agregar-evaluados',
  templateUrl: './agregar-evaluados.component.html',
  styleUrls: ['./agregar-evaluados.component.scss'],
})
export class AgregarEvaluadosComponent implements OnInit {
  evaluados: IParticipanteEvaluador[];
  evaluadosSeleccionados: IParticipanteEvaluador[];
  form: FormGroup;

  constructor(
    private matDialog: MatDialogRef<AgregarEvaluadosComponent>,
  ...
  ngOnInit(): void {
    this.servidoresRepository
      .listarEvaluadosSinEvaluador(this.data.evaluador)
      .subscribe((res) => {
        this.evaluados = res;
      });
  }
  ...
  guardar() {
    ...
    if (this.form.valid) {
      ...
      this.servidoresRepository
        .agregarMandoMedioEvaluado(mandoMedio, evaluados)
        .subscribe((r) => {

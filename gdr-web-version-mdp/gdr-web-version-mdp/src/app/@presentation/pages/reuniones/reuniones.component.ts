...
@Component({
  selector: 'serv-talento-reuniones',
  templateUrl: './reuniones.component.html',
  styleUrls: ['./reuniones.component.scss'],
})
export class ReunionesComponent implements OnInit, AfterViewInit {
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  paginationSizes: number[] = [10, 20, 50, 100];
  defaultPageSize = this.paginationSizes[0];
  displayedColumns: string[] = ['descripcionEvaluado', 'descripcionPuesto', 'fecha', 'hora', 'duracion', 'acciones'];
  ...
  constructor(
    ...
    if (rolId) {
      ...
      this.authenticationService.getApplicationRoles().subscribe(item => {
        rolId = JSON.parse(sessionStorage.getItem('orden_rol_id'));
        this.validarEvaluado(String(rolId));
      });
    }
  ...
  validarEvaluado(rolId: string) {
    if (rolId === '2') {
      ...
      this.organigramaRepository.getListPuesto(this.ciclo.entidadId, persona?.personaId)
        .subscribe(x => {
          if (x && x.length > 0) {
            let puesto = x[0];
            sessionStorage.setItem('nombreEvaluado', persona?.nombreCompleto);
            // this.router.navigate(['/pages/reuniones/' + puesto.detalleuoId + "/" + puesto.detUnidadOrganicaEvaluadorId]);
            this.redirectHistorialEvaluados();
          } else {
            this.toastService.showToast('No se ha encontrado puestos del evaluado', 'danger');
          }
        });
    }
  }
  ...
  openEditReunion(elem) {
    ...
    let fecha = new Date();
    let hora = (fecha.getHours().toString().length === 1 ?  '0'.concat(fecha.getHours().toString()) : fecha.getHours().toString()) + ":" + (fecha.getMinutes().toString().length === 1 ? '0'.concat(fecha.getMinutes().toString()) : fecha.getMinutes().toString());
    ...
    this.openFormReunion(false, true);
  }
  ...
  openFormReunion(createMode: boolean = true, reprogramarMode: boolean = false) {
  ...
  verHistorial(elem) {
    ...
    this.router.navigate(['/pages/reuniones/' + elem?.evaluadoDetalleUoId + "/" + this.puestoTab?.detalleUoId]);
  }

...
/*
      ...
      if (valorCaptcha === getHash(this.loginForm.get('inputCaptcha').value)) {
        ...
        this.authenticationService.login3(userForm, passwordForm).subscribe(
          (res) => {
            ...
            if (this.token) {
              listaEntidad = this.authenticationService
                .getEntidades(this.token)
                .subscribe((item) => {
                  listaEntidad = item;
                  this.authenticationService
                    .getListaEntidades(listaEntidad)
                    .subscribe((lista) => {
                      if (lista.payload.entidad.length > 1) {
                        this.openModalEntidad(
                          lista.payload.entidad,
                          this.rolId
                        );
                      } else {
                        if (lista.payload.entidad[0].entidadId !== null) {
                          this.getObtenerRoles(
                            lista.payload.entidad[0].entidadId,
                            this.rolId
                          );
                        } else {
                          throw new Error(
  ...
  getObtenerRoles(entidadId, rolId) {
    ...
    if (this.rolId !== null) {
      this.authenticationService
        .getRolEntidad(entidadId, this.rolId)
        .subscribe((item) => {
          if (item.length === 1) {

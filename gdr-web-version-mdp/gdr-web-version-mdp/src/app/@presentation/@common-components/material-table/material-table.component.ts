...
export class MaterialTableComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    if (this.checkboxes) {
      this.displayedColumns.unshift('Checkboxes');
    }
    if (this.acciones && this.permissionRepository.isReadAndWrite) {
  ...
  setTableDataSource(data: any) {
    ...
    this.selectedItems = this.tableDataSource.data.map(x => false);
    this.defaultSelectRows = this.tableDataSource.data.filter(item => item[this.keyValueSelect]);
    this.selection = new SelectionModel<any>(true, this.defaultSelectRows);
    if (this.headerCheck) {
  ...
  getNotSelected() {
    return this.tableDataSource.data.filter(item => !this.selection.selected.includes(item));
  }
}
